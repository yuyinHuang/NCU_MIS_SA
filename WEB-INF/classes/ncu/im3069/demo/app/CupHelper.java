...
public class CupHelper {
    ...
    private static CupHelper ch;
    private Connection conn = null;
    private PreparedStatement pres = null;
    
    public static CupHelper getHelper() {
        /** Singleton檢查是否已經有ProductHelper物件，若無則new一個，若有則直接回傳 */
        if(ch == null) ch = new CupHelper();
    ...
    public JSONObject update(Cup c) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`cup` SET `name` = ? ,`image` = ? , `price` = ? , `quantity` = ? WHERE `id` = ?";
            /** 取得所需之參數 */
            int id = c.getID();
    ...
    public JSONObject deleteByID(int id) {
        ...
        try {
            ...
            /** SQL指令 */
            String sql = "DELETE FROM `missa`.`cup` WHERE `id` = ? LIMIT 1";
